...
function DOM_request( method, url, data, callback, options = {} )
{
	...
	let requestOptions = { method, credentials: 'include', headers : new Headers({'Content-Type': methods[ method ], 'Cache-Control' : 'no-cache', 'x-requested-with' : 'XMLHttpRequest' }) };
	...
	fetch( url, requestOptions )
		.then( async ( response ) =>
		{
			...
			else
			{
				...
				{
					if( text[0] === '{' && text[ text.length - 1 ] === '}' ){ try { return JSON.parse( text ); } catch (e) { return text; } }
					return text;
				} )
			}
		...
		.then( ( response ) =>
		{
			if( response.status === 401 ){ location.reload(); /*Unauthorized*/ }
			callback( response );
		})
		.catch(e =>
		{
